"""
DeepSeek-V4-Pro Document AI integration for OCR processing.
Provides an alternative to Azure Document Intelligence and Mistral Document AI
using DeepSeek's Document AI API.
"""
import base64
import logging
import httpx
...
def get_deepseek_doc_ai_client(cosmos_config_container=None):
    """
    Get DeepSeek-V4-Pro Document AI configuration from environment.
    ...
    deepseek_endpoint = config.get("deepseek_doc_ai_endpoint")
    deepseek_api_key = config.get("deepseek_doc_ai_key")
    deepseek_model = config.get("deepseek_doc_ai_model", "deepseek-v4-pro")

    if not deepseek_endpoint or not deepseek_api_key:
        raise ValueError(
            "DeepSeek-V4-Pro Document AI endpoint and key must be configured. "
            "Set DEEPSEEK_DOC_AI_ENDPOINT and DEEPSEEK_DOC_AI_KEY environment variables."
        )
...
def get_ocr_results(file_path: str, cosmos_config_container=None, json_schema: Optional[dict] = None) -> str:
    """
    Extract text from document using DeepSeek-V4-Pro Document AI.

    This function provides the same interface as Azure Document Intelligence
    and Mistral Document AI to allow seamless switching between OCR providers.
    ...
    deepseek_config = get_deepseek_doc_ai_client(cosmos_config_container)
    ...
    try:
        with httpx.Client(timeout=300.0) as client:  # 5 minute timeout for large documents
            ...
            logger.info(f"[Thread-{thread_id}] DeepSeek-V4-Pro Document AI response received")

            # Extract markdown/text content from response
            ocr_text = ""
            ...
            logger.info(f"[Thread-{thread_id}] DeepSeek-V4-Pro Document AI OCR completed, {len(ocr_text)} characters")
            return ocr_text

    except httpx.HTTPStatusError as e:
        logger.error(f"[Thread-{thread_id}] DeepSeek-V4-Pro API HTTP error: {e.response.status_code}")
        logger.error(f"[Thread-{thread_id}] Response: {e.response.text}")
        raise Exception(f"DeepSeek-V4-Pro Document AI API error: {e.response.status_code} - {e.response.text}")
    except httpx.RequestError as e:
        logger.error(f"[Thread-{thread_id}] DeepSeek-V4-Pro API request error: {str(e)}")
        raise Exception(f"DeepSeek-V4-Pro Document AI request failed: {str(e)}")
    except Exception as e:
        logger.error(f"[Thread-{thread_id}] Unexpected error during DeepSeek-V4-Pro Document AI processing: {str(e)}")
        raise
